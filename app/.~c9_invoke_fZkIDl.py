...
def details(idnum):
    ...
    return render_template('details.html', hit=hit,event=event, user_id=user_id,event_id=event_id,users=users,contact=contact)
...
def rating_sent():
  ...
  try:
      ...
      return jsonify(result=rating_num)
################################################################################
# Statistics And What_if Analysis
################################################################################

@app.route('/test/<idnum>', methods=['GET'])
def stats(idnum):
    ratings = []
    for i in range(5,0,-1):
        ratings.append(Rating.query.filter_by(event_id=idnum,rate=i).count())
    # cpositve = Comment.query.with_entities(func.sum(Comment.positive).label('sum')).filter(Comment.event_id==idnum)
    # cnegative = Comment.query.with_entities(func.sum(Comment.negative).label('sum')).filter(Comment.event_id==idnum)
    # negative = cnagative.scalar()
    # positive = cpositive.scalar()
    i = 1
    days=[]
    visits =[]
    hits = Hit.query.with_entities(func.sum(Hit.number).label('sum')).group_by(Hit.timestamp).filter(Hit.event_id==idnum).all()
    for hit in hits:
        visits.append(hit.sum)
        days.append("day"+str(i))
        i=i+1
    return render_template('test.html',ratings=json.dumps(ratings),visits=json.dumps(visits),days=days)

@app.route('/what_if_analysis/<int:ev_id>', methods=['GET'])
def what_if_analysis(ev_id):
    event = db.session.query(Event).filter_by(creator=g.user).filter_by(id=ev_id).first()
    views = db.session.query(Hit).filter_by(event_id=ev_id).all()
    ratings = db.session.query(Rating).filter_by(event_id=Event.id).filter(Event.creator==g.user).filter_by(event_id=ev_id).all()
    pay = db.session.query(Payment).filter_by(event_id=ev_id).all()
    
    views_users_id=[]
    views_event_id=[]
    for j in range(len(views)):
        views_users_id.append(views[j].users_id)
        views_event_id.append(views[j].event_id)
    
    ratings_users_id=[]
    ratings_event_id=[]
    for r in range(len(ratings)):
        rating_users_id.append(rating[r].users_id)
        rating_event_id.append(rating[r].event_id)
    
    beta_views={}
    for i in range(len(pay)):
        if pay[i].event_id in views_event_id and pay[i].users_id in view_user_id:
            for k in range(len(views)):
                if pay[i].users_id == views[k].users_id:
                    beta_views.append(view[k].number)
    
    pdb.set_trace()
    return render_template('what_if_analysis.html', event=event, views=views, ratings=ratings, pay=pay)
###############################################################################

################################################################################
# User sign up or login Start
################################################################################
