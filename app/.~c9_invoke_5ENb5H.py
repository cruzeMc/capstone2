...
def events_listing(cat):
    ...
    if form.validate_on_submit():
        ...
        card = db.session.query(Card).filter_by(id=form.card_num.data, user_id=g.user)
        event = db.session.query(Event).filter_by(id=form.event_num.data)
        pay = make_payment(g.user, str(user.first_name), str(user.last_name), card.payment_method, card.card_type, card.id, card.expire_month, card.expire_year, event.event_name, form.event_num.data, event.price, form.qty.data)
        if pay==True:
            return redirect(url_for('success'))
        else:
             k = str(pay['details'][0]['field']) + ": " + str(pay['details'][0]['issue'])
             return render_template('failure.html', k=k)
    return render_template('events.html', event_lst=event_lst, category=cat, form=form, card_lst=card_lst)
...
def account():
    ...
    user_id = g.user
    users = Users.query.filter_by(id=user_id).first()
    return render_template('account.html', users=users, form=form)
...
def welcome():
    category_list = db.session.querysxxxz(Category).all()
    user_id = g.user
    user = db.session.query(Users).filter_by(id=user_id).first()
