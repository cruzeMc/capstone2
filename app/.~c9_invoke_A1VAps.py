...
def newevent():
    ...
    return render_template('createEvent.html', lst=form=form)
...
def details(idnum):
    ...
    if request.method == "POST":
        ...
        comment = Comment(user_id,event_id,form.post.data,status,percentage)
        db.session.add(comment)
        db.session.commit()
        return redirect(url_for('details',id=id))
    return render_template('details.html', ylist=ylist,comments=comments)
...
def signup():
    ...
    if request.method=='POST':
        ...
        filename=secure_filename(img.filename)
        insert_user(form.first_name.data, form.last_name.data, form.usersname.data, filename, form.email.data,form.password.data,form.address.data,)
...
def comment(id):
    ...
    form = CommentForm()
    if request.method == 'GET':
        return render_template('comment.html', username=username,form=form,id=id,comments=comments)
    if form.validate():
        status = sentiment(form.post)[0]
        percentage = str(sentiment(form.post)[1]*100)[:5]
        comment = Comment(user_id,event_id,form.post.data,status,percentage)
